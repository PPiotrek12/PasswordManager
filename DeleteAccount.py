...
class DeleteAccount(Accounts):
    ...
    def deleteAccount(self):
        ...
        if not decision:
            Label(self.delete_account_frame, text = "Accunt hasn't been deleted.", fg = "red").grid(row = len(self.servicesList) + 3, column = 0)
            return
    ...
    def __init__(self, root, menu_frame):
        ...
        text = "Select which service\n you would like to delete", font = "Helvetica 20 bold").grid(row = 0, column = 0, pady = 20)
        ...
        btnSee = Button(self.delete_account_frame, text = "Delete", command = self.deleteAccount, width = 14)
